...
export function formatErrorGroupSummary(errorGroup: ErrorGroupStats): string {
  const { group, count, affectedUsersCount, firstSeenTime, lastSeenTime, representative } = errorGroup;
 ...
 */
export function analyseErrorPatternsAndSuggestRemediation(errorGroups: ErrorGroupStats[]): string {
  ...
  const totalErrors = errorGroups.reduce((sum, group) => sum + parseInt(group.count), 0);
  const totalAffectedUsers = errorGroups.reduce((sum, group) => sum + parseInt(group.affectedUsersCount), 0);
  ...
  const sortedErrors = [...errorGroups].sort((a, b) => parseInt(b.count) - parseInt(a.count));
  ...
  if (serviceGroups.size > 1) {
    ...
    serviceGroups.forEach((groups, service) => {
      const serviceErrors = groups.reduce((sum, group) => sum + parseInt(group.count), 0);
...
function suggestRemediation(errorGroup: ErrorGroupStats): string {
  ...
  const httpStatus = errorGroup.representative?.context?.httpRequest?.responseStatusCode;
  ...
  if (httpStatus) {
    if (httpStatus >= 500 && httpStatus < 600) {
      suggestions.push("• **Server Error (5xx):** Check application logs for internal errors, database connectivity, and resource availability");
      suggestions.push("• Monitor application health metrics and consider scaling if under high load");
    } else if (httpStatus >= 400 && httpStatus < 500) {
      suggestions.push("• **Client Error (4xx):** Review API documentation and input validation");
      if (httpStatus === 404) {
        suggestions.push("• **404 Not Found:** Verify routing configuration and resource existence");
      } else if (httpStatus === 401 || httpStatus === 403) {
        suggestions.push("• **Authentication/Authorisation:** Check API keys, tokens, and permission configurations");
      }
    }
  }
  ...
  if (message.includes('timeout') || message.includes('deadline')) {
    suggestions.push("• **Timeout Issues:** Increase timeout values, optimise query performance, or implement retry logic");
    suggestions.push("• Consider using asynchronous processing for long-running operations");
  }
  
  if (message.includes('memory') || message.includes('oom')) {
    suggestions.push("• **Memory Issues:** Increase memory allocation, optimise memory usage, or implement memory profiling");
    suggestions.push("• Review for memory leaks and consider garbage collection tuning");
  }
  
  if (message.includes('database') || message.includes('sql')) {
    suggestions.push("• **Database Issues:** Check database connectivity, query performance, and connection pool settings");
    suggestions.push("• Monitor database metrics and consider query optimisation");
  }
  
  if (message.includes('permission') || message.includes('access denied')) {
    suggestions.push("• **Permission Issues:** Verify IAM roles, service account permissions, and resource access policies");
    suggestions.push("• Check Google Cloud IAM configuration for the affected service");
  }
  
  if (message.includes('rate limit') || message.includes('quota')) {
    suggestions.push("• **Rate Limiting:** Review API quotas, implement exponential backoff, and consider request batching");
    suggestions.push("• Monitor usage patterns and request quota increases if needed");
  }
  
  if (message.includes('connection') || message.includes('network')) {
    suggestions.push("• **Network Issues:** Check network connectivity, firewall rules, and DNS resolution");
    suggestions.push("• Consider implementing circuit breaker pattern for external dependencies");
  }
  
  // Generic suggestions if no specific patterns found
  if (suggestions.length === 0) {
    suggestions.push("• **General:** Enable detailed logging to capture more context about this error");
    suggestions.push("• Review recent deployments and configuration changes");
    suggestions.push("• Monitor error frequency and implement alerting if not already in place");
  }
  
  // Add investigation steps
  suggestions.push("• **Investigation:** Use Error Reporting to view recent occurrences and stack traces");
  suggestions.push("• Check related logs in Cloud Logging for additional context");
